...
class UnalignedSequenceLength(AbstractFilter):
	def __init__(self):
		self.var_mult = 2
	...
	def initialize(self, source_language, target_language, extra_args):
		...
		self.model_filename = "models/" + extra_args['input filename'] + "__UnalignedSequenceLength.stats"
		if self.normalize:
	...
	def finalize(self):
		...
		f.write("UnalignedSequenceLength")
	...
	def decide(self, tu):
		...
		if self.normalize:
			src_mean = 1.0 - (min(src_mean, 4.0) / 4.0)
			trg_mean = 1.0 - (min(trg_mean, 4.0) / 4.0)
